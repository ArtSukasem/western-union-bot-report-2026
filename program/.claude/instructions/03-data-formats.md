...
- `ScanRsp` matches **any file** (regardless of extension, including none) whose
  name starts with `RSP_Inbound` / `RSP_Outbound` in `input-rsp/`; `ScanTxnReport`
  matches names starting with `Transaction-Report` in `input-transaction-report/`.
  Extension doesn't matter — EPPlus reads by content, not by file extension.
