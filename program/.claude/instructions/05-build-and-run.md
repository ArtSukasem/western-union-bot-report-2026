...
from the build output directory to find `input-rsp/`, `lookups/`, `mapper/`, and
...
`AppConfig` defaults. `MainForm.EnsureFolders()` creates `input-rsp/`,
`input-transaction-report/`, `lookups/`, `mapper/`, and `report-results/` if
missing, so the user has somewhere to drop files even on a fresh checkout.
