...
- On load and on "🔄 โหลดใหม่" (reload), scans `input-rsp/` and
  `input-transaction-report/` via `InputFolderScanner`, storing the full
  unfiltered list of IB/OB/Transaction Report files.
...
1. Drop this month's `RSP_Inbound_Mmmyy.xlsx` / `RSP_Outbound_Mmmyy.xlsx` /
   `Transaction-Report Mmmyy.xls` into `input-rsp/` /
   `input-transaction-report/`, plus prior months' RSP files needed for Rules
   202/203's rolling windows.
